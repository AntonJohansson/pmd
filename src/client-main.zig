...
const draw = @import("client/draw.zig");
const cl = @import("client/command_meta.zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    const io = init.io;
    core.init(args, io, thread_loop);
}

// Draw state
var command_buffer: gs.draw_api.CommandBuffer = .{};

// Player state
var local_players: gs.BoundedArray(LocalPlayer, 16) = .{};

//
// main-loop
//

fn thread_loop(module: *const core.Module, io: std.Io) void {
    var log = gs.thread.log_memory.group_log(.general);
    log.info("Starting thread {}", .{gs.thread.id});

    var window: ?*c.GLFWwindow = null;

    if (gs.thread.is_main()) {
        gs.memory.animation_states = gs.ArrayCircular(gs.AnimationState){
            .data = gs.thread.arena_persistent.alloc(gs.AnimationState, 64),
        };
        gs.memory.windows.arena = &gs.thread.arena_frame;
    ...
    core.thread_await_main(gs.thread);

    if (!module.function_table.init(gs.system, gs.thread, gs.memory)) {
        log.err("Failed to initialize module: {s}", .{module.name});
        return;
    }
    defer module.function_table.deinit(gs.thread, gs.memory);

    // Load all sounds into buffers
    var playing_sounds: gs.BoundedArray(PlayingSound, 64) = .{};
    ...
    @memset(&gs.memory.console_input.data, 0);

    var old_mouse_pos: v2 = .{ .x = 0.0, .y = 0.0 };

    while (gs.thread.should_run.load(.acquire)) {
        core.frame_begin(gs.thread, gs.memory, io);

        if (gs.thread.is_main()) {
            gs.memory.windows.head = null;
            gs.memory.windows.tail = null;
            gs.memory.map_mods.used = 0;
        }
        ...
        if (gs.thread.is_main()) {
            ...
            gs.memory.time += gs.system.desired_frame_time;

            if (c.glfwWindowShouldClose(window) == 1) {
                core.thread_exit_all();
            }

            c.glfwGetWindowSize(window, &width, &height);
            gs.config.vars.aspect = @as(f32, @floatFromInt(width)) / @as(f32, @floatFromInt(height));
        }
        ...
        if (gs.thread.is_main()) {
            ...
            if (server_index) |si| {
                const block_rescv = gs.thread.profile.begin("net receive", 0);
                events = net.receiveMessagesClient(&host, si);
                gs.thread.profile.end(block_rescv);

                //
                // Process network data
                //
                const block_process = gs.thread.profile.begin("net process", 0);
                for (events) |event| {
                    switch (event) {
                        .peer_connected => {
                        ...
                        .message_received => |e| {
                            switch (e.kind) {
                                .Command => {
                                    const message: *align(1) net.packet.Command = @ptrCast(e.data);
                                    ...
                                    gs.command.dodododododododo(message.data[0..message.len]);
                                },
                                .ConnectionTimeout => {
                                    const message: *align(1) net.packet.ConnectionTimeout = @ptrCast(e.data);
                                    _ = message;
                                    if (connected) {
                                        connected = false;
                                ...
                                .Joined => {
                                    const message: *align(1) net.packet.Joined = @ptrCast(e.data);
                                    gs.mutex_system.lock();
                                    gs.memory.tick = message.tick;
                                    gs.mutex_system.unlock();
                                    log.info("Joined message", .{});
                                },
                                .PlayerJoinResponse => {
                                    const message: *align(1) net.packet.PlayerJoinResponse = @ptrCast(e.data);
                                    gs.memory.players.append(.{
                                        .id = message.id,
                                    });
                                ...
                                .PeerJoined => {
                                    const message: *align(1) net.packet.PeerJoined = @ptrCast(e.data);
                                    gs.memory.players.append(message.player);
                                    log.info("Player connected {}", .{message.player.id});
                                },
                                .PeerDisconnected => {
                                    const message: *align(1) net.packet.PeerDisconnected = @ptrCast(e.data);
                                    const index = gs.findIndexById(gs.memory.players.slice(), message.id);
                                    if (index != null)
                                        _ = gs.memory.players.swap_remove(index.?);
                                    log.info("Player {} disconnected", .{message.id});
                                },
                                .SpawnPlayer => {
                                    const message: *align(1) net.packet.SpawnPlayer = @ptrCast(e.data);
                                    const player = gs.findPlayerById(&gs.memory.players.data, message.player.id) orelse unreachable;
                                    player.* = message.player;
                                    log.info("Spawning", .{});
                                },
                                .PlayerUpdateAuth => {
                                    const message: *align(1) net.packet.PlayerUpdateAuth = @ptrCast(e.data);
                                    const local_player = findLocalPlayerById(local_players.slice(), message.player.id);
                                    if (local_player != null) {
                                        const player = gs.findPlayerById(&gs.memory.players.data, message.player.id);

                                        var auth_player = message.player;
                                        var offset = @as(i64, @intCast(message.tick)) - @as(i64, @intCast(gs.memory.tick)) + 2;
                                        var auth_memory = gs.memory.*;
                                        while (offset <= 0) : (offset += 1) {
                                            const old_input = local_player.?.input_buffer.peekRelative(offset);
                                            module.function_table.update(gs.thread, &gs.config.vars, &auth_memory, &auth_player, old_input, gs.system.dt);
                                        }
                                ...
                                .ServerPlayerUpdate => {
                                    const message: *align(1) net.packet.ServerPlayerUpdate = @ptrCast(e.data);
                                    for (message.players[0..message.num_players]) |player| {
                                        if (findLocalPlayerById(local_players.slice(), player.id) != null)
                                            continue;
                                        if (gs.findPlayerById(&gs.memory.players.data, player.id)) |p| {
                                            p.* = player;
                                        } else {
                                            gs.memory.players.append(player);
                                        }
                                    }
                                },
                                .NewSounds => {
                                    const message: *align(1) net.packet.NewSounds = @ptrCast(e.data);
                                    for (message.new_sounds[0..message.num_sounds]) |s| {
                                        ...
                                        gs.memory.new_sounds.append(s);
                                    }
                                },
                                .NewHitscans => {
                                    const message: *align(1) net.packet.NewHitscans = @ptrCast(e.data);
                                    for (message.new_hitscans[0..message.num_hitscans]) |h| {
                                        ...
                                        if (local)
                                            continue;
                                        gs.memory.new_hitscans.append(h);
                                    }
                                },
                                .NewExplosions => {
                                    const message: *align(1) net.packet.NewExplosions = @ptrCast(e.data);
                                    appendSliceAssumeForceDstAlignment(1, gs.Explosion, &gs.memory.new_explosions, message.new_explosions[0..message.num_explosions]);
                                },
                                .NewNades => {
                                    const message: *align(1) net.packet.NewNades = @ptrCast(e.data);
                                    appendSliceAssumeForceDstAlignment(1, gs.Nade, &gs.memory.new_nades, message.new_nades[0..message.num_nades]);
                                },
                                .NewDamage => {
                                    const message: *align(1) net.packet.NewDamage = @ptrCast(e.data);
                                    appendSliceAssumeForceDstAlignment(1, gs.Damage, &gs.memory.new_damage, message.new_damage[0..message.num_damage]);
                                },
                                .NewMapMods => {
                                    std.log.info("received map mods from server", .{});
                                    const message: *align(1) net.packet.NewMapMods = @ptrCast(e.data);
                                    for (message.mods[0..message.num_mods]) |m| {
                                        gs.memory.map_mods.append(m);
                                    }
                                },
                                .Kill => {
                                    const message: *align(1) net.packet.Kill = @ptrCast(e.data);
                                    var entry = gs.memory.killfeed.push();
                                    entry.from = message.from;
                                    entry.to = message.to;
                                    entry.time_left = 2.0;
                                },
                                .EntityUpdate => {
                                    const message: *align(1) net.packet.EntityUpdate = @ptrCast(e.data);
                                    if (gs.findEntityById(gs.memory.entities.slice(), message.entity.id)) |entity| {
                                        entity.* = message.entity;
                                    } else {
                                        gs.memory.entities.append(message.entity);
                                    }
                                },
                                else => {
                ...
                gs.thread.profile.end(block_process);
            }
        }
        ...
        if (gs.thread.is_main()) {
            ...
            //
            {
                ...
                {
                    {
                        const block = gs.thread.profile.begin("new player", 0);
                        defer gs.thread.profile.end(block);
                                ...
                                if (i == keyboard_input_device_id) {
                                    ...
                                    if (key_pressed or mouse_moved) {
                                        ...
                                        // if we're not connected.
                                        {
                                            var lp = LocalPlayer{};
                                            if (!connected) {
                                                lp.id = gs.newEntityId();
                                            } else {
                                                lp.id = null;
                                            }
                                            lp.input_device_id = i;
                                            lp.input_maps = .{};
                                            setup_input_maps(&gs.memory.pack, &lp.input_maps.?, .mouse_keyboard);

                                            if (server_index) |index| {
                                                net.pushMessage(index, net.packet.PlayerJoinRequest{});
                                            }
                                    ...
                                    if (present == 1) {
                                        ...
                                        for (gamepad.buttons) |b| {
                                            const action: gs.res.Action = @enumFromInt(b);
                                            if (action == .press) {
                                                gamepad_press = true;
                                                occupied_input_devices[i] = .occupied;
                                                log.info("Gamepad {}", .{i});
                                                ...
                                                // if we're not connected.
                                                {
                                                    ...
                                                    lp.input_device_id = i;
                                                    lp.input_maps = .{};
                                                    setup_input_maps(&gs.memory.pack, &lp.input_maps.?, .gamepad);
                    ...
                    {
                        const block = gs.thread.profile.begin("gather input", 0);
                        defer gs.thread.profile.end(block);
                        ...
                        for (local_players.slice()) |*lp| {
                            ...
                            // Collect inputs if we're not in the console
                            {
                                ...
                                gs.memory.active_state = active_state(lp);
                                const input_map = lp.input_maps.?.get(active_state(lp));

                                for (&input_map.map, 0..) |*state, i| {
                                    const input_name: gs.InputName = @enumFromInt(i);
                                    //if (memory.state == .console and input_name != .Enter) {
                                    //    continue;
                                    //}

                                    const action: gs.res.Action = switch (state.input_type) {
                                        gs.res.InputType.key => |key| glfw_get_key(window, key),
                                        gs.res.InputType.mouse_button => |mb| @enumFromInt(c.glfwGetMouseButton(window, @intFromEnum(mb))),
                                        gs.res.InputType.mouse_scroll => |ms| switch (ms.dir) {
                                            .scroll_down => if (scroll_delta < 0.0) .press else .release,
                                            .scroll_up => if (scroll_delta > 0.0) .press else .release,
                                        },
                                        gs.res.InputType.gamepad_button => |gb| blk: {
                                        ...
                                        gs.res.InputType.gamepad_axis_abs => |a| blk: {
                                            const deadzone = 0.1;
                                            const index = (lp.input_device_id.? + max_num_gamepads - debug_gamepad_off) % max_num_gamepads;
                                            var gamepad: c.GLFWgamepadstate = undefined;
                                            if (c.glfwGetGamepadState(@intCast(index), &gamepad) == 0) {
                            ...
                            if (lp.input_device_id.? == keyboard_input_device_id) {
                                ...
                                lp.input.cursor_delta = .{ .x = gs.config.vars.sensitivity * delta.x, .y = gs.config.vars.sensitivity * delta.y };
                            } else {
                                ...
                                var count: c_int = undefined;
                                const axes = c.glfwGetJoystickAxes(@intCast(index), &count);
                                var dx = axes[@intFromEnum(gs.res.GamepadAxis.right_x)];
                                var dy = axes[@intFromEnum(gs.res.GamepadAxis.right_y)];
                                var abs_dx = @abs(dx);
                                var abs_dy = @abs(dy);
                                ...
                                lp.input.cursor_delta.x = 20.0 * gs.config.vars.sensitivity * dx / abs_dx;
                                // TODO(anjo): Move aspect ratio to camera
                                lp.input.cursor_delta.y = (9.0 / 16.0) * 20.0 * gs.config.vars.sensitivity * dy / abs_dy;
                            }

                            // push input state
                            lp.input_buffer.push(lp.input);

                            const player = gs.findPlayerById(&gs.memory.players.data, lp.id.?);

                            if (connected) {
                                net.pushMessage(server_index.?, net.packet.PlayerUpdate{
                                    .tick = gs.memory.tick,
                                    .id = lp.id.?,
                                    .input = lp.input,
                            ...
                            if (player != null) {
                                {
                                    const block_update = gs.thread.profile.begin("update", 0);
                                    defer gs.thread.profile.end(block_update);
                                    module.function_table.update(gs.thread, &gs.config.vars, gs.memory, player.?, &lp.input, gs.system.dt);
                                }
                                if (!connected) {
                                    module.function_table.authorizedPlayerUpdate(gs.thread, &gs.config.vars, gs.memory, player.?, &lp.input, gs.system.dt);
                                }
                            }
                            ...
                            if (capture_text) {
                                ...
                                const diff = gs.memory.time - key_repeat_timer;
                                if (last_key_down and (!repeat and diff >= 500 * std.time.ns_per_s or repeat and diff >= 50 * std.time.ns_per_s)) {
                                    key_repeat_timer = gs.memory.time;
                                    repeat = true;
                                }

                                if (gs.memory.console_input_index > 0 and (glfw_get_key(window, .left) == .press or last_key == .left and repeat)) {
                                    gs.memory.console_input_index -= 1;
                                }
                                if (gs.memory.console_input_index < gs.memory.console_input.used and (glfw_get_key(window, .right) == .press or last_key == .right and repeat)) {
                                    gs.memory.console_input_index += 1;
                                }
                                if (gs.memory.console_input_index > 0 and (glfw_get_key(window, .backspace) == .press or last_key == .backspace and repeat)) {
                                    _ = gs.memory.console_input.ordered_remove(gs.memory.console_input_index - 1);
                                    gs.memory.console_input.data[gs.memory.console_input.used] = 0;
                                    gs.memory.console_input_index -= 1;
                                }
                                if (gs.memory.console_input.used < gs.memory.console_input.data.len - 1 and (last_char >= 32 and last_char <= 126 and repeat)) {
                                    gs.memory.console_input.append(@as(u8, @truncate(@as(u32, @intCast(last_char)))));
                                    gs.memory.console_input.data[gs.memory.console_input.used] = 0;
                                    gs.memory.console_input_index += 1;
                                }

                                if (lp.input.isset(.Enter) and gs.memory.console_input.used > 0) {
                                    gs.command.dodododododododo(gs.memory.console_input.slice());
                                    std.log.info("Running command: {s}", .{gs.memory.console_input.slice()});

                                    gs.memory.console_input_index = 0;
                                    gs.memory.console_input.used = 0;
                                    gs.memory.console_input.data[0] = 0;
                                }
                            }
                    ...
                    for (gs.memory.new_sounds.slice()) |s| {
                        _ = s;
                    ...
                    gs.memory.new_sounds.clear();

                    for (gs.memory.new_hitscans.slice()) |h| {
                        gs.memory.hitscans.append(h);
                    }
                    gs.memory.new_hitscans.clear();

                    for (gs.memory.new_nades.slice()) |n| {
                        gs.memory.nades.append(n);
                    }
                    gs.memory.new_nades.clear();

                    for (gs.memory.new_explosions.slice()) |e| {
                        gs.memory.explosions.append(e);
                    }
                    gs.memory.explosions.clear();

                    gs.memory.new_damage.clear();

                    gs.memory.new_spawns.clear();
                }
            }
        }
        ...
        if (gs.thread.is_main()) {
            ...
            if (!connected) {
                module.function_table.authorizedUpdate(gs.thread, &gs.config.vars, gs.memory, gs.system.dt);
            }

            module.function_table.client_update(gs.thread, &gs.config.vars, gs.memory, gs.system.dt);

            // Send updated entities to server
            if (connected) {
                for (gs.memory.entities.slice()) |*e| {
                    if (!e.flags.updated_client)
                        continue;
                    e.flags.updated_client = false;
                    net.pushMessageToAllPeers(net.packet.EntityUpdate{
                        .entity = e.*,
                    });
                }
            }
        ...
        if (gs.thread.is_main()) {
            ...
            //
            if (server_index) |index| {
                net.process(&host, index, gs.memory.time);
            }
        }
        ...
        {
            {
                const block = gs.thread.profile.begin("draw collect", 0);
                defer gs.thread.profile.end(block);
                for (local_players.slice()) |lp| {
                    if (lp.id == null) {
                        continue;
                    }
                    module.function_table.draw(gs.thread, &gs.config.vars, gs.memory, &command_buffer, lp.id.?, &lp.input);
                }
            }
        }
        ...
        if (gs.thread.is_main()) {
            {
                const block = gs.thread.profile.begin("draw", 0);
                defer gs.thread.profile.end(block);
        ...
        if (gs.thread.is_main()) {
            ...
            //
            {
                ...
                var samples = gs.thread.arena_frame.alloc(f32, num_needed_samples);
                @memset(samples, 0);
        ...
        core.frame_end(gs.thread, gs.memory, io);
    }

    log.info("Exiting thread {}", .{gs.thread.id});
}
...
pub fn update_game_state(lp: *LocalPlayer, input: *const gs.Input) void {
...
pub fn active_state(lp: *LocalPlayer) gs.State {
    return lp.state_stack[lp.active_state];
}

pub fn push_state(lp: *LocalPlayer, s: gs.State) void {
    std.debug.assert(lp.active_state + 1 < lp.state_stack.len);
    lp.active_state += 1;
    lp.state_stack[lp.active_state] = s;
}

pub fn pop_state(lp: *LocalPlayer) gs.State {
...
fn setup_mouse_keyboard_input(im: *gs.res.InputMaps) void {
...
fn setup_gamepad_input(im: *gs.res.InputMaps) void {
    const gameplay = im.get(.gameplay);
...
fn setup_input_maps(pack: *gs.goosepack.Pack, im: *gs.res.InputMaps, t: InputMapsType) void {
    _ = pack;
    switch (t) {
...
const LocalPlayer = struct {
    const num_inputs = @typeInfo(gs.InputName).@"enum".fields.len;
    const num_states = @typeInfo(gs.State).@"enum".fields.len;

    active_state: u8 = 0,
    state_stack: [num_states]gs.State = .{.gameplay} ** num_states,
    from_state: gs.State = .gameplay,

    input_device: ?InputDevice = null,
    input_device_id: ?usize = null,
    input_maps: ?gs.res.InputMaps = null,
    input: gs.Input = undefined,

    last_input_actions: [num_inputs]gs.res.Action = .{.release} ** num_inputs,

    // TODO: We only need to allocate this when on network
    input_buffer: gs.bb.CircularBuffer(gs.Input, 256) = .{},

    id: ?gs.EntityId = null,
};

fn findLocalPlayerById(lp: []LocalPlayer, id: gs.EntityId) ?*LocalPlayer {
    for (lp) |*l| {
...
fn glfw_get_key(window: ?*c.GLFWwindow, key: gs.res.Key) gs.res.Action {
    return @enumFromInt(c.glfwGetKey(window, @intFromEnum(key)));
}

var key_repeat_timer: u64 = 0;
var last_key: gs.res.Key = undefined;
var last_char: u8 = 0;
var repeat = false;
...
fn charCallback(window: ?*c.GLFWwindow, codepoint: c_uint) callconv(.c) void {
    ...
    if (gs.memory.console_input.used < gs.memory.console_input.data.len - 1) {
        gs.memory.console_input.append(@as(u8, @truncate(@as(u32, @intCast(last_char)))));
        gs.memory.console_input.data[gs.memory.console_input.used] = 0;
        gs.memory.console_input_index += 1;
    }
